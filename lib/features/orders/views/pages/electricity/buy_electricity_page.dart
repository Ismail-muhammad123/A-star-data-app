...
class _PurchaseElectricityFormPageState
  ...
  _purchase() async {
    ...
    try {
      if (_saveBeneficiary && _meterNumberController.text.isNotEmpty) {
        try {
          await OrderServices().savePurchaseBeneficiary(
            ...
            PurchaseBeneficiary(
              ...
              identifier: _meterNumberController.text,
              nickname: meterDetails['Customer Name'] ?? _meterNumberController.text,
            ),
          );
        } catch (e) {
  ...
  Future<void> _fetchBeneficiaries() async {
    try {
      ...
      if (token != null) {
        ...
        if (mounted) {
          setState(() {
            _beneficiaries = fetched.where((b) => b.serviceType == 'electricity').toList();
          });
        }
      }
    } catch (e) {
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return GestureDetector(
                            onTap: () {
                              setState(() {
                                _meterNumberController.text = ben.identifier;
                                _isVerified = false;
                                _saveBeneficiary = false;
                              });
                              // Since Meter Type is required, don't auto-verify unless it's selected.
                              if (_meterType != null) {
                                _verifyMeter();
                              }
                            },
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                borderRadius: BorderRadius.circular(12),
                                border: Border.all(
                                  color: _meterNumberController.text == ben.identifier
                                      ? Colors.blueAccent
                                      : Colors.transparent,
                                  width: 2,
                                ),
